...
struct ResettaPrimaryButtonStyle: ButtonStyle {
        ...
        configuration.label
            ...
            .background {
                RoundedRectangle(cornerRadius: 18, style: .continuous)
                    .fill(ResettaTheme.accent.opacity(isEnabled ? 1 : 0.36))
            }
...
struct ResettaQuietButtonStyle: ButtonStyle {
    @Environment(\.accessibilityReduceMotion) private var reduceMotion
    ...
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .background {
                RoundedRectangle(cornerRadius: 18, style: .continuous)
                    .fill(ResettaTheme.quietFill.opacity(configuration.isPressed ? 0.64 : 0))
            }
...
struct ResettaOptionButtonStyle: ButtonStyle {
    @Environment(\.accessibilityReduceMotion) private var reduceMotion
        ...
        configuration.label
            ...
            .background {
                RoundedRectangle(cornerRadius: 17, style: .continuous)
                    .fill(isSelected ? ResettaTheme.accent : ResettaTheme.surface)
            }
            .overlay {
                RoundedRectangle(cornerRadius: 17, style: .continuous)
                    .stroke(isSelected ? ResettaTheme.accent.opacity(0.42) : ResettaTheme.subtleLine, lineWidth: 1)
            }
            .scaleEffect(reduceMotion ? 1 : (configuration.isPressed ? 0.985 : 1))
...
struct ResettaIconButtonStyle: ButtonStyle {
    @Environment(\.accessibilityReduceMotion) private var reduceMotion
    ...
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .background(ResettaTheme.surface, in: Circle())
            .overlay {
                Circle()
